...
# Genera dati casuali per Tigri e gatti
np.random.seed(42)
...
# Dividi i dati normalizzati in Tigri e gatti
...
labels = np.array([0] * 50 + [1] * 50)  # Etichette: 0 per Tigri, 1 per gatti
...
plt.scatter(dogs_x_normalized, dogs_y_normalized, color='blue', label='Tigri')
...
plt.legend()
plt.title('Classificazione di Tigri e Gatti (dati normalizzati)')
plt.show()
